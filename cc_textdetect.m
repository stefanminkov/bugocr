...
function boxes=cc_textdetect(I)
    [rowc,colc,~] = size(I);
    gray = rgb2gray(I);
    
    % Preprocessing steps
    % Noise reduction
    gfilter = fspecial('gaussian',10,2);
...
%     figure; imshow(after_threshold); title('Results of adaptive threshold');
    ...
    after_removing_noise = bwareaopen(after_threshold,100);
